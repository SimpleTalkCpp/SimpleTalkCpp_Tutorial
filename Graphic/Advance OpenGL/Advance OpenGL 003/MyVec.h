...
public:
	...
	MyVec2f operator-() const { return MyVec2f(-x, -y); }
};
...
public:
	...
	MyVec3f operator-() const { return MyVec3f(-x, -y, -z); }
	...
	MyVec3f min(const MyVec3f& r) const {
		return MyVec3f(std::min(x, r.x), std::min(y, r.y), std::min(z, r.z));
	}

	MyVec3f max(const MyVec3f& r) const {
		return MyVec3f(std::max(x, r.x), std::max(y, r.y), std::max(z, r.z));
	}
};
...
public:
	...
	MyVec4f operator-() const { return MyVec4f(-x, -y, -z, -w); }
};
