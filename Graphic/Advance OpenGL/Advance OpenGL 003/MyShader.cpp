...
void MyShader::bind() {
	...
	assert(m_boundTexCount == 0);
}

void MyShader::unbind() {
	for (int i = 0; i < m_boundTexCount; i++) {
...
void MyShader::setUniform(const char* name, const MyTexture2D& tex) {
	if (m_boundTexCount >= kMaxSamplers) {
		throw MyError("too many samplers");
	}
...
void MySampler::create() {
	if (!m_p) {
		...
		glSamplerParameterf(m_p, GL_TEXTURE_WRAP_S, GL_REPEAT);
		glSamplerParameterf(m_p, GL_TEXTURE_WRAP_T, GL_REPEAT);
		glSamplerParameterf(m_p, GL_TEXTURE_WRAP_R, GL_REPEAT);

		glSamplerParameterf(m_p, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
		glSamplerParameterf(m_p, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
	}
}

void MySampler::destroy() {
	if (m_p) {
		glDeleteSamplers(1, &m_p);
		m_p = 0;
	}
}
